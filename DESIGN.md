...
- **不举报也会处理**：逾期满 `OVERDUE_AUTO_DEFAULT_H`（默认 72 小时）仍未付清、也未登记已付的记录，系统提前 24 小时警告发布方一次，然后自动执行与"A 类成立"完全相同的处置（名下逾期记录全部转违约、欠款公示、任务关闭、列入黑名单）；该发布方名下尚未交小法庭的 A 类举报随之成立结案。误伤（已付未登记）走 F 类黑名单申诉；接单方在 `payable` / `overdue` 状态也可直接点「已收到」完成，付了没登记的发布方不会因此上榜。（2026-09-24 用户拍板：线上出现多个逾期一周无人举报的发布方。）
...
stateDiagram-v2
  ...
  overdue --> defaulted: 逾期满 72h 自动违约上榜
...
| `overdue` | 逾期未付 | 发布方 | 举报后 24h 宽限；满 72h 自动违约上榜（提前 24h 警告）；期间发布方被冻结 | |
